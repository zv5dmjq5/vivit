"""Generate plots of tuples ``(|γₖ|, λₖ)`` during training."""
...
def get_ylimits(data):
    ...
    ymin = min(numpy.min(numpy.abs(m["gammas_mean"])) for m in traverse(data))
    ymax = max(numpy.max(m["gammas_mean"]) for m in traverse(data))

    return ymin, ymax


def plot(config):
    """Plot absolute value of mean directional gradient over eigenvalues."""
    ...
    for checkpoint, metric in traverse(data, checkpoint=True):
        ...
        gammas_mean = metric["gammas_mean"]
        lambdas_mean = metric["lambdas_mean"]
        ...
        plt.plot(
            lambdas_mean, numpy.abs(gammas_mean), marker="o", linestyle="", alpha=0.5
        )
